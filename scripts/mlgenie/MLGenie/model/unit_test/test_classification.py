...
from MLGenie.design.hpo_param_config import ClsParamConfig


class TestClsModel(unittest.TestCase):
    """Test ClsModel class"""
    ...
    def setUp(self) -> None:
        ...
        self.fit_performance_threshold = 0.0  # 0.65
    ...
    def test_init(self) -> None:
        ...
        for model_name in self.model_list:
            ...
            with self.assertRaises(AssertionError):
                svm_model = model_name(metrics="Accuracy")

            with self.assertRaises(AssertionError):
                svm_model = model_name(metrics=Metrics)

            # Test invalid average value
            with self.assertRaises(AssertionError):
                svm_model = model_name(average="average")

            with self.assertRaises(AssertionError):
                svm_model = model_name(average=None)
    ...
    def test_HPO(self) -> None:
        ...
        for model_name in self.model_list:
            model = model_name(
                hpo_algorithm=HPOAlgorithm.GridSearch, hpo_search_iter=100
            )
            ...
            model = model_name(
                hpo_algorithm=HPOAlgorithm.RandomSearch, hpo_search_iter=50
            )
            best_params, best_score, cv_results = model._HPO(self.X, self.y)
            self.assertTrue(isinstance(best_params, dict))
            self.assertTrue(isinstance(best_score, float))
    ...
    def test_fit(self) -> None:
        ...
        for model_name in self.model_list:
            ...
            with self.assertRaises(ValueError):
                model = model_name(cv=int(self.y.value_counts().min()) + 1)
                model.fit(self.X, self.y)
